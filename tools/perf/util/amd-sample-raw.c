...
static bool zen4_ibs_extentions;

static void set_zen4_ibs_extensions(struct perf_env *env)
{
	u32 eax = 0, ebx = 0, ecx = 0, edx = 0;

	if (perf_env__find_cpuid_leaf(env, 0x8000001B, 0x00, &eax, &ebx, &ecx, &edx))
		return;

	zen4_ibs_extentions = !!(eax & IBS_CAPS_ZEN4IBSEXTENSIONS);
}
