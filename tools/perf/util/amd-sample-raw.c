...
{
	static const char * const l1tlb_pgsz_str[] = {
		" L1TlbPgSz 4KB",
		" L1TlbPgSz 2MB",
		" L1TlbPgSz 1GB",
		" L1TlbPgSz RESERVED"
	...
	printf("ibs_fetch_ctl:\t%016llx MaxCnt %7d Cnt %7d Lat %5d En %d Val %d Comp %d IcMiss %d "
	       "PhyAddrValid %d%s L1TlbMiss %d L2TlbMiss %d RandEn %d%s\n",
		reg.val, reg.fetch_maxcnt << 4, reg.fetch_cnt << 4, reg.fetch_lat,
		reg.fetch_en, reg.fetch_val, reg.fetch_comp, reg.ic_miss, reg.phy_addr_valid,
		reg.phy_addr_valid ? l1tlb_pgsz_str[reg.l1tlb_pgsz] : "",
...
static void pr_ibs_op_data3(union ibs_op_data3 reg)
{
	...
	printf("ibs_op_data3:\t%016llx LdOp %d StOp %d DcL1TlbMiss %d DcL2TlbMiss %d "
		...
		reg.dc_l2_tlb_hit_1g, reg.l2_miss, reg.sw_pf, op_mem_width_str,
...
static u32 ibs_fetch_type, ibs_op_type;
...
{
	int ret, nr_pmu_mappings = evlist->env->nr_pmu_mappings;
	char *pmu_mapping = evlist->env->pmu_mappings;
