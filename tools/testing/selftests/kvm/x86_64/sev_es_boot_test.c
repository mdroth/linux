...
static void *ghcb0 = NULL;

static void vc_handler(struct ex_regs *regs)
{
	sev_es_handle_vc(ghcb0, regs);
}

static void __attribute__((__flatten__))
guest_code(void *ghcb, struct sev_sync_data *sync, uint8_t *private_buf)
{
...
int main(int argc, char *argv[])
{
	...
	kvm_sev_ioctl(vm, sev_fd, KVM_SEV_ES_INIT, NULL);
	...
	/* Set up our VCPU and #VC handler */
	...
	/* Set up guest params, leave the first page of shared mem for GHCB */
	sync = addr_gva2hva(vm, shared_vaddr + PAGE_SIZE);
	vcpu_args_set(vm, VCPU_ID, 3, shared_vaddr, shared_vaddr + PAGE_SIZE,
		      private_vaddr);

	/* Get ready to encrypt initial state */
	ksev_launch_start.policy = 0x4;
	...
	TEST_ASSERT(ksev_guest_status.policy == 0x4, "incorrect guest policy");
	...
	kvm_sev_ioctl(vm, sev_fd, KVM_SEV_LAUNCH_UPDATE_VMSA, NULL);
