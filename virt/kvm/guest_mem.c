...
static long kvm_gmem_allocate(struct file *file, int mode, loff_t offset,
			      loff_t len)
{
	struct address_space *mapping = file->f_mapping;
			       ...
			       loff_t len)
{
	...
	if (mode & FALLOC_FL_PUNCH_HOLE)
		ret = kvm_gmem_punch_hole(file, mode, offset, len);
	else
		ret = kvm_gmem_allocate(file, mode, offset, len);
...
void kvm_gmem_unbind(struct kvm_memory_slot *slot)
{
	...
	if (WARN_ON_ONCE(!slot->gmem.file))
		return;
