...
static __init int rmptable_init(void)
{
	...
	if (!rmp_base || !rmp_end) {
		pr_err("SEV-SNP: Memory for the RMP table is not reserved by the BIOS\n");
		return 1;
	}
...
skip_enable:
	...
	pr_info("SEV-SNP: RMP table physical 0x%016llx - 0x%016llx\n", rmp_base, rmp_end);
	return 0;
}
