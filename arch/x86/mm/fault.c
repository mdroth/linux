			 ...
			 unsigned long address)
{
	...
	if (unlikely(error_code & X86_PF_RSVD)) {
		/*
		 * This may not return, go ahead and disable tracing around
		 * the pgtable_bad() call.
		 */
			...
			unsigned long address)
{
	...
	hw_pf_tracing_enter();
	rc = __do_user_addr_fault(regs, error_code, address);
	hw_pf_tracing_exit(rc);
}
