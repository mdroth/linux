...
static bool amd_pmu_has_legacy_overflow(int idx)
{
	u64 counter;
...
static bool amd_pmu_has_global_overflow(int idx)
{
	return amd_pmu_get_global_overflow() & BIT_ULL(idx);
}

DEFINE_STATIC_CALL(amd_pmu_has_overflow, amd_pmu_has_legacy_overflow);
...
static int __init amd_core_pmu_init(void)
{
	...
	if (boot_cpu_has(X86_FEATURE_PERFMON_V2)) {
		...
		static_call_update(amd_pmu_has_overflow, amd_pmu_has_global_overflow);
	}
