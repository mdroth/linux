...
static __always_inline u64 amd_pmu_lbr_get_from(unsigned int idx)
{
	...
	rdmsrl(MSR_AMD64_LBR_FROM + idx * 2, val);
...
static __always_inline u64 amd_pmu_lbr_get_to(unsigned int idx)
{
	...
	rdmsrl(MSR_AMD64_LBR_TO + idx * 2, val);

	return val;
}

static __always_inline u64 amd_pmu_lbr_branch_ip(u64 entry)
{
	u32 shift = 64 - boot_cpu_data.x86_virt_bits;

	return (u64)(((s64)entry << shift) >> shift);
}

static __always_inline bool amd_pmu_lbr_is_mispredict(u64 entry)
{
	return entry & LBR_ENTRY_MISPREDICT;
}
...
void amd_pmu_lbr_read(void)
{
	...
	for (i = 0; i < x86_pmu.lbr_nr; i++) {
		...
		br[out].from	= amd_pmu_lbr_branch_ip(from);
...
void amd_pmu_lbr_enable_all(void)
{
	...
	wrmsrl(MSR_IA32_DEBUGCTLMSR, dbg_ctl | DEBUGCTLMSR_FLBRI);
