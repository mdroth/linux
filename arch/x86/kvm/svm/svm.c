...
static void init_vmcb(struct vcpu_svm *svm)
{
	...
	if (boot_cpu_has(X86_FEATURE_V_SPEC_CTRL))
		set_msr_interception(&svm->vcpu, svm->msrpm, MSR_IA32_SPEC_CTRL, 1, 1);
...
static void svm_vcpu_load(struct kvm_vcpu *vcpu, int cpu)
{
	...
	if (sev_es_guest(svm->vcpu.kvm)) {
	    ...
	    asm volatile(__ex("vmsave %0")
                     : : "a" (__sme_page_pa(sd->save_area)) : "memory");
	}
					...
					struct vcpu_svm *svm)
{
	...
	if (sev_es_guest(svm->vcpu.kvm)) {
	    ...
	    asm volatile(__ex("vmload %0") : : "a" (__sme_page_pa(sd->save_area)));
	}
...
static void svm_vcpu_after_set_cpuid(struct kvm_vcpu *vcpu)
{
	...
	/* only guests that use global asid get to use
	 * INVLGB in guests
	 */
