...
static int sev_put_memfile_pfn_handler(struct kvm *kvm,
				       struct kvm_gfn_range *range, void *data)
{
	...
	for (gfn = range->start; gfn < range->end; gfn++) {
		if (!pages[i])
			continue;
		kvm_private_mem_put_pfn(memslot, page_to_pfn(pages[i++]));
	}
...
{
	return kvm_vm_do_hva_range_op(kvm, addr, size,
				      sev_put_memfile_pfn_handler, pages);
}
...
e_unpin:
	if (kvm_is_upm_enabled(kvm)) {
		sev_put_memfile_pfn(kvm, vaddr, size, npages, inpages);
	} else {
...
static void sev_rmp_page_level_adjust_upm(struct kvm *kvm, gfn_t gfn, int *level)
{
	...
	/* No risk of shared hugepages overlapping with private pages */
	if (!kvm_mem_is_private(kvm, gfn)) {
		pr_debug("%s: shared: gfn: %llx, level: %d\n", __func__, gfn, *level);
		return;
	}
...
static void handle_rmp_page_fault_upm(struct kvm_vcpu *vcpu, gpa_t gpa, u64 error_code)
{
	 ...
	 * PFERR_GUEST_SIZEM_MASK might not be set. In those cases it just be
...
void handle_rmp_page_fault(struct kvm_vcpu *vcpu, gpa_t gpa, u64 error_code)
{
	...
	/* TODO: with UPM this should only happen when a PSMASH is needed */
	if (kvm_is_upm_enabled(vcpu->kvm)) {
		pr_debug("%s: called for gpa: %llx, error_code: %llx", __func__,
			 gpa, error_code);
		return handle_rmp_page_fault_upm(vcpu, gpa, error_code);
	}
			...
			gfn_t start, gfn_t end)
{
	...
	enum psc_op op = (attr & KVM_MEM_ATTR_PRIVATE) ? SNP_PAGE_STATE_PRIVATE
	...
	while (gfn < end) {
		 ...
		 * level argument.
		 */
		if (attr == KVM_MEM_ATTR_PRIVATE && order &&
