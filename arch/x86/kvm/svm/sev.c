...
static bool snp_active __read_mostly;
...
static int snp_launch_finish(struct kvm *kvm, struct kvm_sev_cmd *argp)
{
	...
	/* Measure all vCPUs using the LAUNCH_UPDATE before we finalize the launch flow. */
...
static void __unregister_enc_region_locked(struct kvm *kvm,
					   struct enc_region *region)
{
	unsigned long i, pfn, base_pfn;
	struct rmpupdate val = {};
	rmpentry_t *e;
	int level, rc;

	/*
	 * On SEV-SNP, the guest memory pages are assigned in the RMP table. Unassigned it
	...
	if (sev_snp_guest(kvm)) {
		for (i = 0; i < region->npages; i++) {
			...
			if (!e) {
				pr_err("SNP: failed to read RMP entry (pfn 0x%lx\n", pfn);
				continue;
			}
			...
			if (level == PG_LEVEL_2M) {
				...
				base_pfn = pfn & ~(KVM_PAGES_PER_HPAGE(PG_LEVEL_2M) - 1);
			} else {
				val.pagesize = RMP_PG_SIZE_4K;
			}

			/* Transition the page to hypervisor owned. */
			rc = rmptable_rmpupdate(pfn_to_page(base_pfn), &val);
			if (rc)
				pr_err("SNP: failed to release pfn 0x%lx ret=%d\n", base_pfn, rc);
		}
	}
...
static void kvm_snp_probe(void)
{
	u64 val;
...
static int snp_page_shared(struct kvm_vcpu *vcpu, gpa_t gpa, kvm_pfn_t pfn, int level)
{
	struct rmpupdate val;
	int rc, rmp_level;
	rmpentry_t *e;
...
static int snp_page_private(struct kvm_vcpu *vcpu, gpa_t gpa, kvm_pfn_t pfn, int level)
{
	...
	/*
	 * Log if the page is already validated. A guest should have invalidated it before
	 * requesting the page state change.
	 */
	if (rmpentry_validated(e))
...
static int __snp_handle_page_state_change(struct kvm_vcpu *vcpu, int op, gpa_t gpa, int level)
{
	...
	for (; end > gpa; gpa = next_gpa) {
		...
		case SNP_PAGE_STATE_SHARED:
			rc = snp_page_shared(vcpu, gpa, pfn, level);
			break;
		case SNP_PAGE_STATE_PRIVATE:
			rc = snp_page_private(vcpu, gpa, pfn, level);
			break;
...
static unsigned long snp_handle_page_state_change(struct kvm_vcpu *vcpu,
						  struct snp_page_state_change *info)
{
	struct snp_page_state_entry *entry = &info->entry[info->header.cur_entry];
	unsigned long rc;
	int level, op;
	gpa_t gpa;
...
static int sev_handle_vmgexit_msr_protocol(struct vcpu_svm *svm)
{
	...
	case GHCB_MSR_GPA_REGISTER_REQ: {
		kvm_pfn_t pfn;
		u64 gfn;

		gfn = get_ghcb_msr_bits(svm,
					GHCB_MSR_GPA_REGISTER_VALUE_MASK,
					GHCB_MSR_GPA_REGISTER_VALUE_POS);

		pfn = kvm_vcpu_gfn_to_pfn(vcpu, gfn);
		if (is_error_noslot_pfn(pfn))
			gfn = GHCB_MSR_GPA_REGISTER_ERROR;
		else
		...
		set_ghcb_msr_bits(svm, gfn,
				  GHCB_MSR_GPA_REGISTER_VALUE_MASK,
				  GHCB_MSR_GPA_REGISTER_VALUE_POS);
		set_ghcb_msr_bits(svm, GHCB_MSR_GPA_REGISTER_RESP,
				  GHCB_MSR_INFO_MASK,
				  GHCB_MSR_INFO_POS);
		break;
	}
...
int sev_handle_vmgexit(struct vcpu_svm *svm)
{
	...
	case SVM_VMGEXIT_PAGE_STATE_CHANGE: {
		...
		rc = snp_handle_page_state_change(&svm->vcpu, info);
