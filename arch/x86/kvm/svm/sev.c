...
static int sev_private_mem_get_pfn_handler(struct kvm *kvm, struct kvm_gfn_range *range,
					   void *data)
{
...
static int sev_private_mem_get_pfn(struct kvm *kvm, unsigned long addr,
				   unsigned long size, unsigned long npages,
				   struct page **pages)
{
	return kvm_vm_do_hva_range_op(kvm, addr, addr + size,
				      sev_private_mem_get_pfn_handler, pages);
}
				    ...
				    int write)
{
	...
	if (kvm_arch_has_private_mem(kvm)) {
		...
		if (sev_private_mem_get_pfn(kvm, uaddr, ulen, npages, pages)) {
