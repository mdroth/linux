...
static int amd_ir_set_vcpu_affinity(struct irq_data *data, void *vcpu_info)
{
	...
	if (pi_data->is_guest_mode) {
		...
		irte->lo.fields_remap.destination = APICID_TO_IRTE_DEST_LO(cfg->dest_apicid);
		irte->hi.fields.destination = APICID_TO_IRTE_DEST_HI(cfg->dest_apicid);
...
int amd_iommu_update_ga(int cpu, bool is_run, void *data)
{
	...
	if (ref->lo.fields_vapic.guest_mode) {
		if (cpu >= 0) {
			ref->lo.fields_vapic.destination = APICID_TO_IRTE_DEST_LO(cpu);
			ref->hi.fields.destination = APICID_TO_IRTE_DEST_HI(cpu);
		}
