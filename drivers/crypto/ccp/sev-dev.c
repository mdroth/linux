...
static int snp_reclaim_page(struct page *page)
{
...
static int __sev_do_cmd_locked(int cmd, void *data, int *psp_ret)
{
	 ...
	 * In that case use an intermedite command buffer page to complete the
	 * operation.
	 *
	...
	if (sev->snp_inited && sev_legacy_cmd_buf_writable(cmd)) {
		...
		if (ret) {
			dev_err(sev->dev, "sev cmd id %#x, failed to change to "
					...
					ret);
			if (snp_reclaim_page(cmd_page))
				/*
				 * We failed to reclaim the page, do not release the page back to
				 * system since the immutable bit is set for this page.
				 */
				goto skip_free;
		}
	}
...
void sev_pci_init(void)
{
	...
	if (boot_cpu_has(X86_FEATURE_SEV_SNP)) {
		...
		dev_dbg(sev->dev, "SEV-SNP is disabled by the BIOS\n");
	}
